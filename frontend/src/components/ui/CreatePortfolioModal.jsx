...
export default function CreatePortfolioModal({ onClose }) {
  ...
  const [form, setForm] = useState({ name: '', description: '', currency: 'PLN' })
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
      <div className="bg-white border border-gray-200 rounded-xl w-full max-w-md p-6 shadow-lg">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-gray-900 font-semibold">Nowy portfel</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input className="input" placeholder="np. IKE GPW, XTB USD, Obligacje" value={form.name} onChange={(e) => set('name', e.target.value)} autoFocus required />
          </div>
          <div>
            <label className="label">Opis (opcjonalnie)</label>
            <input className="input" placeholder="np. Akcje polskie na koncie IKE" value={form.description} onChange={(e) => set('description', e.target.value)} />
          </div>
          <div>
