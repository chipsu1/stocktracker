...
export default function EditPositionModal({ position, onClose, onSave }) {
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')
  const [form, setForm] = useState({
    ticker: position.ticker || '',
    name: position.name || '',
    asset_class: position.asset_class || 'Akcje',
    currency: position.currency || 'PLN',
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError(err.message || 'Błąd edycji pozycji')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-4">
      <div className="bg-gray-900 border border-gray-800 rounded-xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-white font-semibold">Edytuj pozycję</h2>
